                ...
                <div class="col-12 mb-4">
                    <fieldset class="reset-this redo-fieldset" style="margin-bottom: 10px">
                        ...
                        <div class="row gy-2 gx-3 align-items-center">
                            <div class="col-3">
                                <label class="mb-1" for="department_id">Department</label>
                                <select class="select2-options-department-id form-control"
                                        name="department_id" id="department_id" autocomplete="department_id">
                                </select>
                            </div>
                            <div class="col-3">
                                <label class="mb-1" for="glimpse_category_id">Glimpse Category</label>
                                <select class="select2-options-glimpse-category-id form-control"
                                        name="glimpse_category_id" id="glimpse_category_id"
                                        autocomplete="glimpse_category_id">
                                </select>
                            </div>
                            <div class="col-3">
                            ...
                            <div class="col-3">
                                <label class="mb-1" for="glimpse_day_id">Glimpse Day</label>
                                <select class="select2-options-glimpse-day-id form-control"
                                        name="glimpse_day_id" id="glimpse_day_id"
                                        autocomplete="glimpse_day_id">
                                </select>
                            </div>
                        </div>
...
@endsection
@push('footer-scripts')
    <script src="{{ asset('manager/datatable/datatables.min.js')}}"></script>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#indextable').DataTable({
                ...
                ajax: {
                    "type": "GET",
                    "url": "{{route('manager.get.glimpse')}}",
                    "data": function (d) {
                        d.department_id = document.getElementById('department_id').value;
                        d.glimpse_category_id = document.getElementById('glimpse_category_id').value;
                        d.glimpse_year_id = document.getElementById('glimpse_year_id').value;
                        d.glimpse_day_id = document.getElementById('glimpse_day_id').value;
                    }
                },
                columns: [
                    {data: 'id'},
                    ...
                    {data: 'glimpse_category', orderable: false},
                    {data: 'glimpse_year', orderable: false},
                    {data: 'glimpse_day', orderable: false},
                    {data: null},
                ],
                columnDefs: [
                    {
                    ...
                    },
                    {
                        ...
                        orderable: false,
                        render: function (data, type, row, meta) {
                            let URL = "{{ route('manager.glimpse.show', ':id') }}";
                            URL = URL.replace(':id', row.id);
                            let ACTIVITY = "{{ route('manager.get.glimpse-activity', ':id') }}";
                            ACTIVITY = ACTIVITY.replace(':id', row.id);

                            let view = "{{ route('manager.get-image.glimpse', ':id') }}";
                            view = view.replace(':id', row.id);

                            let download = "{{ route('manager.download-glimpse', ':id') }}";
                            download = download.replace(':id', row.id);

                            return '<div class="d-flex">' +
                                @can('manager_glimpse_glimpse-show')
                                    '<a class="me-1" href="' + URL + '"><span class="badge bg-success text-dark">Show</span>' +
                                @endcan
                                    @can('manager_glimpse_glimpse-edit')
                                    '<a class="me-1" href="' + URL + '/edit"><span class="badge bg-info text-dark">Edit</span></a>' +
                                @endcan
                                    @can('manager_glimpse_glimpse-activity-log')
                                    '<a class="me-1" href="' + ACTIVITY + '"><span class="badge bg-warning text-dark">Activity</span></a>' +
                                @endcan
                                    @can('manager_glimpse_glimpse-delete')
                                ...
                                @endcan
                                    @can('manager_glimpse_glimpse-show')
                                    '<a href="' + view + '" class="me-1" data-toggle="tooltip" data-placement="bottom" title="View File">' +
                                '<span class="badge bg-success"><i class="fa fa-eye text-dark"></i></span>' +
                                '</a>' +
                                @endcan
                                    @can('manager_glimpse_glimpse-download')
                                    '<a href="' + download + '" class="me-1" data-toggle="tooltip" data-placement="bottom" title="Download File">' +
                                '<span class="badge bg-danger"><i class="fa fa-download text-white"></i></span>' +
                                '</a>' +
                                @endcan
                                    '</div>'
