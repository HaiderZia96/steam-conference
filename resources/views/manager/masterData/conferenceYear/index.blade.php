        ...
        <div class="card-body">
            ...
            <div class="d-flex justify-content-between">
                ...
                <div class="btn-toolbar d-none d-md-block" role="toolbar" aria-label="Toolbar with buttons">
                    @can('manager_master-data_conference-year-create')
                        <a href="{{(!empty($url) && isset($url)) ? $url : ''}}"
                           class="btn btn-sm btn-primary">{{(!empty($url_text) && isset($url_text)) ? $url_text : ''}}</a>
                    @endcan
                    @can('manager_master-data_conference-year-activity-log-trash')
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#indextable').DataTable({
                ...
                ajax: {
                    "type": "GET",
                    "url": "{{route('manager.get.conference-year')}}",
                    // "data":function (d){
                ...
                columnDefs: [
                    ...
                    },
                    {
                        ...
                        orderable: false,
                        render: function (data, type, row, meta) {
                            let URL = "{{ route('manager.conference-year.show', ':id') }}";
                            URL = URL.replace(':id', row.id);

                            let STATUS = "{{ route('manager.get.conference-year-status', ':id') }}";
                            ...
                            function generateStatusElement() {
                                ...
                                if (data.status == 1) {
                                    statusElement = '<a class="mx-2" href="' + STATUS + '"><span class="badge bg-success">Active</span></a>';
                                } else {
                                    statusElement = '<a class="mx-2" href="' + STATUS + '"><span class="badge bg-danger">In-Active</span></a>';
                                }

                                return statusElement;
                            }

                            let USER_REGISTRATION = "{{ route('manager.conference-year.user-registration.index',':yid') }}";
                            USER_REGISTRATION = USER_REGISTRATION.replace(':yid', row.id);


                            let ACTIVITY = "{{ route('manager.get.conference-year-activity', ':id') }}";
                            ACTIVITY = ACTIVITY.replace(':id', row.id);

                            let sessionDetail = "{{ route('manager.session-detail', ':yid') }}";
                            sessionDetail = sessionDetail.replace(':yid', row.id);

                            return '<div class="d-flex">' +
                                @can('manager_master-data_conference-year-show')
                                    '<a class="me-1" href="' + URL + '"><span class="badge bg-success text-dark">Show</span>' +
                                @endcan
                                    @can('manager_master-data_conference-year-edit')
                                    '<a class="me-1" href="' + URL + '/edit"><span class="badge bg-info text-dark">Edit</span></a>' +
                                @endcan
                                    @can('manager_master-data_conference-year-activity-log')
                                    '<a class="me-1" href="' + ACTIVITY + '"><span class="badge bg-warning text-dark">Activity</span></a>' +
                                @endcan
                                    @can('manager_master-data_conference-year-delete')
                                ...
                                @endcan
                                    '|' +
                                @can('manager_master-data_conference-year-status-edit')
                                generateStatusElement() +
                                @endcan
                                    '|' +
                                    @can('manager_registration_user-registration-list')
                                    '<a class="mx-2" href="' + sessionDetail + '" data-coreui-toggle="tooltip" data-coreui-placement="top" title="User Registration"> <span class="badge bg-primary text-light">More<i class="fa fa-forward ms-2"></i></span> </a>' +
                                @endcan
                                    // @can('manager_registration_user-registration-list')
                                    //     '| <a class="me-1 ms-2" href="' + sessionDetail + '"><span class="badge bg-info text-white">More >></span></a> ' +
                                    // @endcan
