    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            $('#indextable').DataTable({
                ...
                ajax: {
                    "type":"GET",
                    "url":"{{route('manager.get.payment-type')}}",
                    // "data":function (d){
                ...
                columnDefs: [
                    ...
                    },
                    {
                        ...
                        orderable: false,
                        render: function ( data, type, row, meta ) {
                            let URL = "{{ route('manager.payment-type.show', ':id') }}";
                            URL = URL.replace(':id', row.id);
                            let ACTIVITY = "{{ route('manager.get.payment-type-activity', ':id') }}";
                            ACTIVITY = ACTIVITY.replace(':id', row.id);
                            return '<div class="d-flex">' +
                                @can('manager_master-data_payment-type-show')
                                    '<a class="me-1" href="'+URL+'"><span class="badge bg-success text-dark">Show</span>' +
                                @endcan
                                    @can('manager_master-data_payment-type-edit')
                                    '<a class="me-1" href="'+URL+'/edit"><span class="badge bg-info text-dark">Edit</span></a>' +
                                @endcan
                                    @can('manager_master-data_payment-type-activity-log')
                                    '<a class="me-1" href="'+ACTIVITY+'"><span class="badge bg-warning text-dark">Activity</span></a>' +
                                @endcan
                                    @can('manager_master-data_payment-type-delete')
