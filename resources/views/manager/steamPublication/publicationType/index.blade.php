        ...
        <div class="card-body">
            ...
            <div class="d-flex justify-content-between">
                ...
                <div class="btn-toolbar d-none d-md-block" role="toolbar" aria-label="Toolbar with buttons">
                    @can('manager_steam-publication_publication-type-create')
                        <a href="{{(!empty($url) && isset($url)) ? $url : ''}}" class="btn btn-sm btn-primary">{{(!empty($url_text) && isset($url_text)) ? $url_text : ''}}</a>
                    @endcan
                    @can('manager_steam-publication_publication-type-activity-log-trash')
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            $('#indextable').DataTable({
                ...
                ajax: {
                    "type":"GET",
                    "url":"{{route('manager.get.publication-type')}}",
                    // "data":function (d){
                ...
                columnDefs: [
                    ...
                    },
                    {
                        ...
                        orderable: false,
                        render: function ( data, type, row, meta ) {
                            let URL = "{{ route('manager.publication-type.show', ':id') }}";
                            URL = URL.replace(':id', row.id);
                            let ACTIVITY = "{{ route('manager.get.publication-type-activity', ':id') }}";
                            ACTIVITY = ACTIVITY.replace(':id', row.id);
                            return '<div class="d-flex">' +
                                @can('manager_steam-publication_publication-type-show')
                                    '<a class="me-1" href="'+URL+'"><span class="badge bg-success text-dark">Show</span>' +
                                @endcan
                                    @can('manager_steam-publication_publication-type-edit')
                                    '<a class="me-1" href="'+URL+'/edit"><span class="badge bg-info text-dark">Edit</span></a>' +
                                @endcan
                                    @can('manager_steam-publication_publication-type-activity-log')
                                    '<a class="me-1" href="'+ACTIVITY+'"><span class="badge bg-warning text-dark">Activity</span></a>' +
                                @endcan
                                    @can('manager_steam-publication_publication-type-delete')
