        ...
        <div class="card-body">
            ...
            <form method="{{$method}}" action="{{$action}}" enctype="{{$enctype}}">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" disabled class="form-control @error('contact_no') is-invalid @enderror" name="contact_no" id="contact_no" placeholder="Password" value="{{(isset($data) ? $data->contact_no : old('contact_no'))}}">
                    @error('contact_no')
                    <strong class="text-danger">{{ $message }}</strong>
                    @enderror
                </div>
                <div class="mb-3">
                    <label class="form-label" for="status-type">Paper Submission Status </label>
                    <select disabled class="select2-options-status-type-id form-control @error('status-type') is-invalid @enderror" name="status-type"></select>
                    @error('status-type')
                    <strong class="text-danger">{{ $message }}</strong>
                    @enderror
                </div>
...
@push('footer-scripts')
    ...
    <script>
        $(document).ready(function() {
            //Get Status Type
            let status_type=[{
                id: "{{$data['status_type_id']}}",
                text: "{{$data['status_type_name']}}",
            }];
            $(".select2-options-status-type-id").select2({
                data: status_type,
                theme: "bootstrap5",
